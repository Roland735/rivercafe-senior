...
export async function POST(req) {
    try {
        ...
        if (action === "prepare") {
            const tryPrepareOnModel = async (Model) => {
                const candidateOrders = await Model.find({
                ...
                for (const oRaw of candidateOrders) {
                    const orderDoc = await Model.findById(oRaw._id);
                    ...
                    if (itemPrepared >= itemQty) continue;

                    const overallPrepared = Number(orderDoc.meta?.preparedCount || 0);
                    if (overallPrepared >= totalQty) continue;
            ...
            const resStd = await tryPrepareOnModel(Order);
            if (resStd) return resStd;

            const resSpec = await tryPrepareOnModel(SpecialOrder);
            if (resSpec) return resSpec;
        ...
        // UNPREPARE
        if (action === "unprepare") {
            const tryUnprepareOnModel = async (Model) => {
                const candidateOrders = await Model.find({
            ...
            const resStd = await tryUnprepareOnModel(Order);
            if (resStd) return resStd;

            const resSpec = await tryUnprepareOnModel(SpecialOrder);
            if (resSpec) return resSpec;
